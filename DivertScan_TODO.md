...
**Last updated: Tuesday, August 18, 2026.** Replaces the July 9 version.
...
## 🚧 NOT IN THIS PROJECT — separate builds
- **Ranger Waste dispatch system** (dispatch-to-driver, container inventory,
  customer scheduling, auto-texting — a Box Tracker replacement) lives in its
  own Claude project, own GitHub repo, own Supabase project. Do NOT add it here
  and do NOT put its tables in the DivertScan database. If the two ever need to
  talk, it's an API call between two systems, never a shared DB. Ranger stays an
  approved hauler here; that's unrelated to the software build.
- **Metropolitan demo** DOES belong here — same product, same repo
  (`divertscan-demo.html`), just a sales instance.
